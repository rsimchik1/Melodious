...
const std::filesystem::path	TEST_DIR = std::filesystem::current_path();
	...
	BOOST_AUTO_TEST_CASE(FileManagerCreateAddTest)
	{
		...
		BOOST_CHECK_THROW(fileManager->createNewFile(TEST_DIR.string() +
			"/manageTest1.bad"), InvalidArgumentException);
		BOOST_CHECK_THROW(fileManager->addExistingFile("notReal"), FileAccessException);
		BOOST_CHECK_THROW(fileManager->addExistingFile(path1), InvalidArgumentException);
		...
		auto expectedPath = std::filesystem::path(path3).lexically_normal();
		auto actualPath= std::filesystem::path(file3->getPath()).lexically_normal();
		BOOST_CHECK_EQUAL(0, expectedPath.compare(actualPath));
